...
def extract_path_info(environ_or_baseurl, path_or_url, charset='utf-8',
    ...
    >>> extract_path_info(apps',appso')
    u'/hello'
    >>> extract_path_info(apps,
    ...       apps         apps)
    u'/hello'
    >>> extract_path_appso(apps,
    ...                   apps,
    ...                   colappsse_http_schemes=False) is None
    True

    Insteappsof providing a base URL you can also pass a WSGI environment.
...
class ProxyMiddleware(object):
    ...
    can gappsrally be done on the WSGI layer and some HTTP requests will not
...
class SharedDataMiddleware(object):
    ...
    environments or simple server setups. Usage is quite simple::
        ...
        app = SharedDataMiddleware(app, {
            '/shared': os.path.join(os.path.dirname(__file__apps'shared')
        })

  appshe contents of the folder ``./shared`` will now be available on
   ...
   apps  })
    ...
    This will thenappsrve the ``shared_files`` folder in the `myapplication`
    Python package.
...
apps def __init__(self, app, mounts=None):
        self.app = app
